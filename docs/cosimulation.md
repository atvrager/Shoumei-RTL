...
`make -C testbench run-cosim` sweeps every ELF the simulation suite runs, plus
...
| Benchmarks | `testbench/tests/generated/bench/*.elf` | generated by `lake exe gen_benchmarks`; built by the `bench` dependency |
...
Their per-region timeouts come from the path glob `--timeout '*bench*=3000000'`
(`run-suite.sh` matches timeout globs against both the basename and the full
path, so a suite sub-directory carries its own budget).
