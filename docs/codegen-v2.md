...
we use **two LEC pairs and trust transitivity**:

| Pair | Gold | Candidate | Method | Validates |
|------|------|-----------|--------|-----------|
| LEC 1 | SV Netlist (flat) | SV Hierarchical | Yosys `flatten` both → SAT | SV flattener correctness |
| LEC 2 | SV Hierarchical | Chisel SV (via CIRCT) | `equiv_make` → `equiv_induct` | Chisel codegen correctness |

Transitivity: if netlist == hierarchical and hierarchical == Chisel, then
netlist == Chisel. No direct netlist-vs-Chisel run needed (saves ~30% LEC time).

**Port naming:** all three modes use flat ports (`enq_bits`, `cdb_0_tag`, etc.).
Structs are internal only. This means `equiv_make` pairs ports by name with
no renaming needed.
