...
Sources:
  lean/**/*.lean                        module doc comments, definitions, instances
  GenerateAll.lean                      the canonical circuit registry
  output/sv-from-lean/*.sv              which circuits are actually emitted
  verification/compositional-certs.txt  compositional certificates
...
# Any literal `name := "X"`.  This also matches signal-group records, so callers
# intersect with the emitted-SV name set, which is authoritative for "what is a
# circuit".  Circuits whose name is built by interpolation (`s!"Foo_{n}"`) cannot
# be located this way and are reported as unmapped rather than guessed at.
RE_CIRCUIT_NAME = re.compile(r'name\s*:=\s*"([A-Za-z0-9_]+)"')
...
KNOWN_GAPS = [
    ...
    "Widths are not yet parameters: `CPUConfig.xlen` is unused, 91 data buses in "
    "`CPU.lean` are hard-wired to 32, and the behavioural models use `UInt32`. "
    "FLEN and VLEN are specified in docs/roadmap-rv32g-rv64g.md but not in code.",
    ...
    "Pre-existing FP-store race: `fsw` intermittently enqueues a zero operand "
    "into the store buffer (timing-dependent, value-independent).",
]
...
def main() -> int:
    ...
    emitted = {p.stem for p in SV_DIR.glob("*.sv")} if SV_DIR.exists() else set()
    ...
    out.append("`moduleName :=` references between circuits, certificates from")
    out.append("`verification/compositional-certs.txt`, docs from each file's leading")
