...
- Non-vacuous specification guarantees:
  - `behaviorTraceSpec`: Trace specification induced by a Behavior
  - `Implements.nonVacuous`: Non-triviality via `NonTrivialSpec`
  - `NonVacuousBehavior`: Behavior with non-constant outputs
  - Tautological specifications cannot satisfy non-vacuous requirements
-/
...
structure Implements (reg : ModuleRegistry) (fuel : Nat) (c : Circuit)
    (B : Behavior σ ι ω)
    (absS : State → σ) (encI : ι → Env) (decO : Env → ω) : Prop where
  init_ok : absS initState = B.init
  step_ok : ∀ s i, absS (stepHier reg fuel c s (encI i)).1 = B.step (absS s) i
  out_ok  : ∀ s i, decO (stepHier reg fuel c s (encI i)).2 = B.out (absS s) i
...
/-- Lift a flat sequential circuit proof to `Implements`. -/
theorem implements_of_flat
    ...
    Implements reg (fuel + 1) c B absS encI decO := by
  refine ⟨h_init, fun s i => ?_, fun s i => ?_⟩
...
theorem implements_trace
    ...
    (B : Behavior σ ι ω)
    (absS : State → σ) (encI : ι → Env) (decO : Env → ω)
    (h_impl : Implements reg fuel c B absS encI decO)
    (inputs : Nat → ι) :
  ...
  have h_state : ∀ t, absS (tr.stateAt t) = evalBehaviorState B inputs t := by
    ...
    | succ t ih =>
      ...
      rw [h_next, h_impl.step_ok, ih]
      rfl
  refine ⟨h_state, fun t => ?_⟩
  have h_env : tr.envAt t = (stepHier reg fuel c (tr.stateAt t) (encI (inputs t))).2 := rfl
  rw [h_env, h_impl.out_ok, h_state t]
...
theorem implements_refines_of_flat
    ...
    (B : Behavior σ ι ω)
    (absS : State → σ) (encI : ι → Env) (decO : Env → ω)
    (h_impl : Implements reg (fuel + 1) c B absS encI decO)
  ...
  have h_state : ∀ t, absS (tr.stateAt t) = evalBehaviorState B inputs t := by
    ...
    | succ t ih =>
      have ht := (h_exec.2 t).2
      rw [ht]
      have h_step := h_impl.step_ok (tr.stateAt t) (inputs t)
  ...
  rw [ht_env]
  have h_out := h_impl.out_ok (tr.stateAt t) (inputs t)
