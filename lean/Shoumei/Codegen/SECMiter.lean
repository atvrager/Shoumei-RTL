...
def generateSECMiter (gold rev : Circuit) (miterName : String := s!"{gold.name}_vs_{rev.name}_sec_miter") : String :=
  ...
  let svaBlock := s!"`ifndef SYNTHESIS\n" ++
    s!"  // --------------------------------------------------------------------------\n" ++
...
def generateVCFormalTcl (miterName : String) (files : List String) : String :=
  let readCommands := files.map (fun f => "read_file -format sverilog " ++ f)
  "# Synopsys VC Formal Verification Script\n" ++
  "# Generated by Shoumei Codegen\n\n" ++
  String.intercalate "\n" readCommands ++ "\n\n" ++
  "set_fml_appmode FPV\n" ++
  "elaborate -top " ++ miterName ++ "\n" ++
  "check_fv\n"
