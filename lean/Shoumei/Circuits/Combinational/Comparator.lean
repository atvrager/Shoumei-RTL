...
/-- 64-bit comparator using Subtractor64 (KSA-based internally).
    Computes eq, lt, ltu, gt, gtu in parallel from difference and sign analysis. -/
def mkComparator64 : Circuit :=
  let n := 64
  let a := makeIndexedWires "a" n
  let b := makeIndexedWires "b" n
  ...
  let a_neg_b_pos := Wire.mk "a_neg_b_pos"
  let lt_raw := Wire.mk "lt_raw"
  let lt_gates := [
    Gate.mkAND a_sign b_sign_inv a_neg_b_pos,
    Gate.mkOR a_neg_b_pos same_sign_cmp lt_raw
...
-- Specific width: 64-bit equality-only comparator
def mkEqualityComparator64 : Circuit := mkEqualityComparatorN 64
