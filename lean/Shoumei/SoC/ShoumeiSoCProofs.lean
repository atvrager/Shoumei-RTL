...
/-- Shoumei SoC primary inputs count:
    ...
    + 8 gpio_i + 512 mem_resp_data (one 64 B line) = 526 inputs. -/
theorem shoumei_soc_inputs_count :
    (mkShoumeiSoC defaultCPUConfig).inputs.length = 526 := by native_decide

/-- Shoumei SoC primary outputs count:
    uart_tx, rob_empty, mem_req_valid, mem_req_we (4)
    + 32 mem_req_addr + 512 mem_req_data + 8 gpio_o + 8 gpio_oen = 564 outputs. -/
theorem shoumei_soc_outputs_count :
    (mkShoumeiSoC defaultCPUConfig).outputs.length = 564 := by native_decide
