...
def mkRenameStage (dataWidth : Nat := 32) : Circuit :=
  ...
  -- CRAT instance: dedicated 32x6 register array with restore/dump only
  ...
  let all_outputs :=
    ...
    [rename_valid_1] ++ rs1_phys_out_1 ++ rs2_phys_out_1 ++ rs3_phys_out_1 ++
...
/-- Dual-issue Integer Register Renaming Stage (no rs3, x0 fixed).
    Instantiates CRAT_32x6, 2x IntRAT_32x6, BitmapFreeList_64_W2, IntPhysRegFile_64x{dataWidth}. -/
def mkIntRenameStage (dataWidth : Nat := 64) : Circuit :=
  let tagWidth  := 6
  let archWidth := 5
  ...
  -- Slot 0 inputs (no rs3, no skip_x0_detect)
  let instr_valid_0  := Wire.mk "instr_valid"
  let has_rd_0       := Wire.mk "has_rd"
  let rs1_addr_0     := (List.range archWidth).map fun i => Wire.mk s!"rs1_addr_{i}"
  let rs2_addr_0     := (List.range archWidth).map fun i => Wire.mk s!"rs2_addr_{i}"
  ...
  -- Commit channel 0 (no commit_force_alloc)
  let flush_en            := Wire.mk "flush_en"
  let commit_valid_0      := Wire.mk "commit_valid"
  let commit_archRd_0     := (List.range archWidth).map fun i => Wire.mk s!"commit_archRd_{i}"
  let commit_physRd_0     := (List.range tagWidth).map  fun i => Wire.mk s!"commit_physRd_{i}"
  ...
  -- Commit channel 1 (no commit_force_alloc_1)
  let commit_valid_1      := Wire.mk "commit_valid_1"
  let commit_archRd_1     := (List.range archWidth).map fun i => Wire.mk s!"commit_archRd_1_{i}"
  let commit_physRd_1     := (List.range tagWidth).map  fun i => Wire.mk s!"commit_physRd_1_{i}"
  let commit_hasPhysRd_1  := Wire.mk "commit_hasPhysRd_1"
  ...
  -- Slot 0 outputs (no rs3, no rd_data3)
  let rs1_phys_out_0  := (List.range tagWidth).map fun i => Wire.mk s!"rs1_phys_out_{i}"
  let rs2_phys_out_0  := (List.range tagWidth).map fun i => Wire.mk s!"rs2_phys_out_{i}"
  let rd_phys_out_0   := (List.range tagWidth).map fun i => Wire.mk s!"rd_phys_out_{i}"
  let old_rd_phys_0   := (List.range tagWidth).map fun i => Wire.mk s!"old_rd_phys_out_{i}"
  ...
  let needs_alloc_1 := Wire.mk "needs_alloc_1"
  let needs_alloc_gates :=
    [Gate.mkNOT x0_0 not_x0_0,
     Gate.mkAND has_rd_0 not_x0_0 needs_alloc_0,
     Gate.mkNOT x0_1 not_x0_1,
     Gate.mkAND has_rd_1 not_x0_1 needs_alloc_1]
  ...
  -- CRAT write-enable (integer: always checks not_x0)
  ...
  let retire_gate :=
    [Gate.mkOR  retire_tag_0[0]! retire_tag_0[1]! (Wire.mk "rtnz0_01"),
     ...
     Gate.mkOR  (Wire.mk "rtnz0_0123") retire_tag_0[4]! (Wire.mk "rtnz0_01234"),
     Gate.mkOR  (Wire.mk "rtnz0_01234") retire_tag_0[5]! ret_tag_nz_0,
     Gate.mkAND commit_valid_0 retire_hasPhysRd_0 gated_retire_pre_0,
     Gate.mkAND gated_retire_pre_0 ret_tag_nz_0 gated_retire_0,
     Gate.mkOR  retire_tag_1[0]! retire_tag_1[1]! (Wire.mk "rtnz1_01"),
     Gate.mkOR  (Wire.mk "rtnz1_01") retire_tag_1[2]! (Wire.mk "rtnz1_012"),
     Gate.mkOR  (Wire.mk "rtnz1_012") retire_tag_1[3]! (Wire.mk "rtnz1_0123"),
     Gate.mkOR  (Wire.mk "rtnz1_0123") retire_tag_1[4]! (Wire.mk "rtnz1_01234"),
     Gate.mkOR  (Wire.mk "rtnz1_01234") retire_tag_1[5]! ret_tag_nz_1,
     Gate.mkAND commit_valid_1 retire_hasPhysRd_1 gated_retire_pre_1,
     Gate.mkAND gated_retire_pre_1 ret_tag_nz_1 gated_retire_1]
  ...
  let physregfile_inst : CircuitInstance := {
    moduleName := s!"IntPhysRegFile_64x{dataWidth}"
    instName := "u_prf"
    portMap :=
      [("clock", clock), ("reset", reset),
       ("wr_en_0", prf_wr_en_0), ("wr_en_1", prf_wr_en_1)] ++
      ...
      (rs2_phys_0.enum.map fun ⟨i,w⟩ => (s!"rd_tag2_{i}", w)) ++
      (rs2_phys_1.enum.map fun ⟨i,w⟩ => (s!"rd_tag4_{i}", w)) ++
      (commit_physRd_0.enum.map fun ⟨i,w⟩ => (s!"rd_tag5_{i}", w)) ++
      (commit_physRd_1.enum.map fun ⟨i,w⟩ => (s!"rd_tag6_{i}", w)) ++
      (rs1_phys_1.enum.map fun ⟨i,w⟩ => (s!"rd_tag7_{i}", w)) ++
      (cdb_tag_0.enum.map fun ⟨i,w⟩ => (s!"wr_tag_0_{i}", w)) ++
      (cdb_data_0.enum.map fun ⟨i,w⟩ => (s!"wr_data_0_{i}", w)) ++
      (cdb_tag_1.enum.map fun ⟨i,w⟩ => (s!"wr_tag_1_{i}", w)) ++
  ...
  let all_inputs :=
    ...
    rs1_addr_0 ++ rs2_addr_0 ++ rd_addr_0 ++
    [instr_valid_1, has_rd_1, force_alloc_1] ++
    rs1_addr_1 ++ rs2_addr_1 ++ rd_addr_1 ++
    [flush_en, commit_valid_0] ++ commit_archRd_0 ++ commit_physRd_0 ++
    ...
    [commit_hasPhysRd_1, commit_hasAllocSlot_1,
     cdb_valid_0] ++ cdb_tag_0 ++ cdb_data_0 ++
    [cdb_valid_1] ++ cdb_tag_1 ++ cdb_data_1 ++
    retire_tag_0 ++ retire_tag_1 ++
    [ext_stall, ext_stall_1, retire_hasPhysRd_0, retire_hasPhysRd_1]

  let all_outputs :=
    [rename_valid_0, stall_0] ++ rs1_phys_out_0 ++ rs2_phys_out_0 ++
    rd_phys_out_0 ++ old_rd_phys_0 ++ rs1_data_0 ++ rs2_data_0 ++
    [rename_valid_1] ++ rs1_phys_out_1 ++ rs2_phys_out_1 ++
    rd_phys_out_1 ++ old_rd_phys_1 ++ rs1_data_1 ++ rs2_data_1 ++
    rd_data5 ++ rd_data6
  ...
  { name := if dataWidth == 32 then "IntRenameStage_W2" else s!"IntRenameStage_W2_{dataWidth}"
    inputs := all_inputs
    outputs := all_outputs
    gates := all_gates
    instances := all_instances
...
/-- Single-issue Floating-Point Register Renaming Stage (W=1, with rs3, f0 normal reg).
    Instantiates CRAT_32x6, RAT_32x6, BitmapFreeList_64_W1, FPPhysRegFile_64x{dataWidth}. -/
def mkFPRenameStage (dataWidth : Nat := 64) : Circuit :=
  let tagWidth  := 6
  let archWidth := 5
  ...
  let rs1_addr    := (List.range archWidth).map fun i => Wire.mk s!"rs1_addr_{i}"
  let rs2_addr    := (List.range archWidth).map fun i => Wire.mk s!"rs2_addr_{i}"
  let rs3_addr    := (List.range archWidth).map fun i => Wire.mk s!"rs3_addr_{i}"
  let rd_addr     := (List.range archWidth).map fun i => Wire.mk s!"rd_addr_{i}"

  let flush_en            := Wire.mk "flush_en"
  let commit_valid        := Wire.mk "commit_valid"
  let commit_archRd       := (List.range archWidth).map fun i => Wire.mk s!"commit_archRd_{i}"
  ...
  let old_rd_phys  := (List.range tagWidth).map fun i => Wire.mk s!"old_rd_phys_out_{i}"
  ...
  -- Internal RAT wires
  let rs1_phys   := (List.range tagWidth).map fun i => Wire.mk s!"rs1p_{i}"
  let rs2_phys   := (List.range tagWidth).map fun i => Wire.mk s!"rs2p_{i}"
  let rs3_phys   := (List.range tagWidth).map fun i => Wire.mk s!"rs3p_{i}"
  let rd_phys    := (List.range tagWidth).map fun i => Wire.mk s!"rdp_{i}"
  let old_rd_raw := (List.range tagWidth).map fun i => Wire.mk s!"old_rd_raw_{i}"

  -- f0 is a regular register: always allocate if has_rd
  let needs_alloc := has_rd

  let freelist_enq_ready := Wire.mk "fl_enq_ready"
  let alloc_avail        := Wire.mk "alloc_avail"
  let fl_deq_data        := (List.range tagWidth).map fun i => Wire.mk s!"fl_deq_{i}"
  let fl_deq_ready       := Wire.mk "fl_deq_ready"
  ...
  let control_gates := [
    ...
    Gate.mkAND needs_alloc not_avail stall_raw,
    Gate.mkBUF stall_raw stall,
    Gate.mkOR  stall ext_stall stall_combined,
    Gate.mkNOT stall_combined not_stall,
    Gate.mkAND instr_valid not_stall rename_valid,
    Gate.mkAND needs_alloc rename_valid alloc_fire,
    Gate.mkBUF alloc_fire fl_deq_ready,
    Gate.mkBUF alloc_fire rat_we
  ]

  let rd_phys_gates := (List.range tagWidth).map fun i =>
    Gate.mkAND fl_deq_data[i]! alloc_fire rd_phys[i]!

  -- Committed RAT write logic
  let crat_we := Wire.mk "crat_we"
  let crat_we_enq := Wire.mk "crat_we_enq"
  let crat_ctrl_gates := [
    Gate.mkAND commit_valid commit_hasPhysRd crat_we,
    Gate.mkAND commit_valid commit_hasPhysRd crat_we_enq
  ]
  ...
  let cdec_an := (List.range archWidth).map fun k => Wire.mk s!"cd_an{k}"
  let cdec_not := (List.range archWidth).map fun k =>
    Gate.mkNOT commit_archRd[k]! cdec_an[k]!
  let cdec_outs := (List.range 32).map fun i => Wire.mk s!"cd_{i}"
  let cdec_and := (List.range 32).flatMap fun i =>
    let bits := (List.range archWidth).map fun k =>
      if (i >>> k) &&& 1 == 1 then commit_archRd[k]! else cdec_an[k]!
    let t1 := Wire.mk s!"cd_{i}_t1"
    let t2 := Wire.mk s!"cd_{i}_t2"
    let t3 := Wire.mk s!"cd_{i}_t3"
    [Gate.mkAND bits[0]! bits[1]! t1,
     Gate.mkAND bits[2]! bits[3]! t2,
     Gate.mkAND t1 t2 t3,
     Gate.mkAND t3 bits[4]! cdec_outs[i]!]

  let cd_we := (List.range 32).map fun i => Wire.mk s!"cd_we_{i}"
  let cd_we_gates := (List.range 32).map fun i =>
    Gate.mkAND cdec_outs[i]! crat_we cd_we[i]!

  let crat_merge_gates := (List.range 32).flatMap fun i =>
    (List.range tagWidth).map fun j =>
      Gate.mkMUX crat_raw_dump[i]![j]! commit_physRd[j]! cd_we[i]! crat_dump[i]![j]!

  let crat_gates := cdec_not ++ cdec_and ++ cd_we_gates ++ crat_merge_gates
  ...
  -- Speculative RAT restore logic
  let srat_0_dump := (List.range 32).map fun i =>
    (List.range tagWidth).map fun j => Wire.mk s!"srat0_dump_{i}_{j}"

  let dec_an := (List.range archWidth).map fun k => Wire.mk s!"dec_an{k}"
  let dec_not := (List.range archWidth).map fun k =>
    Gate.mkNOT rd_addr[k]! dec_an[k]!
  let dec_outs := (List.range 32).map fun i => Wire.mk s!"dec_{i}"
  let dec_and := (List.range 32).flatMap fun i =>
    let bits := (List.range archWidth).map fun k =>
      if (i >>> k) &&& 1 == 1 then rd_addr[k]! else dec_an[k]!
    let t1 := Wire.mk s!"dec_{i}_t1"
    let t2 := Wire.mk s!"dec_{i}_t2"
    let t3 := Wire.mk s!"dec_{i}_t3"
    [Gate.mkAND bits[0]! bits[1]! t1,
     Gate.mkAND bits[2]! bits[3]! t2,
     Gate.mkAND t1 t2 t3,
     Gate.mkAND t3 bits[4]! dec_outs[i]!]

  let slot_we := (List.range 32).map fun i => Wire.mk s!"slot_we_{i}"
  let slot_we_gates := (List.range 32).map fun i =>
    Gate.mkAND dec_outs[i]! rat_we slot_we[i]!

  let merged := (List.range 32).map fun i =>
    (List.range tagWidth).map fun j => Wire.mk s!"mrg_{i}_{j}"
  let merged_gates := (List.range 32).flatMap fun i =>
    (List.range tagWidth).map fun j =>
      Gate.mkMUX srat_0_dump[i]![j]! rd_phys[j]! slot_we[i]! merged[i]![j]!

  let final_restore := (List.range 32).map fun i =>
    (List.range tagWidth).map fun j => Wire.mk s!"rst_{i}_{j}"
  let restore_mux_gates := (List.range 32).flatMap fun i =>
    (List.range tagWidth).map fun j =>
      Gate.mkMUX merged[i]![j]! crat_dump[i]![j]! flush_en final_restore[i]![j]!

  let rat_restore_gates := dec_not ++ dec_and ++ slot_we_gates ++ merged_gates ++ restore_mux_gates
  ...
  let freelist_inst : CircuitInstance := {
    ...
    portMap :=
      ...
      [("enq_valid", crat_we_enq),
       ("deq_ready", fl_deq_ready),
       ("flush_en", flush_en),
       ("commit_alloc_en", crat_we)] ++
      (commit_physRd.enum.map fun ⟨i,w⟩ => (s!"commit_alloc_tag_{i}", w)) ++
      [("enq_ready", freelist_enq_ready)] ++
      (fl_deq_data.enum.map fun ⟨i,w⟩ => (s!"deq_data_{i}", w)) ++
  ...
  let out_gates :=
    ...
    (List.range tagWidth).map (fun i => Gate.mkBUF old_rd_raw[i]! old_rd_phys[i]!)
  ...
  let all_gates :=
    ...
    rat_restore_gates ++ out_gates

  let all_instances :=
    [crat_0_inst, rat_inst, freelist_inst, physregfile_inst]

  { name := if dataWidth == 32 then "FPRenameStage_W1" else s!"FPRenameStage_W1_{dataWidth}"
    inputs := all_inputs
    outputs := all_outputs
    gates := all_gates
    instances := all_instances
    signalGroups := [
      { name := "rs1_addr",   width := archWidth, wires := rs1_addr },
      { name := "rs2_addr",   width := archWidth, wires := rs2_addr },
      { name := "rs3_addr",   width := archWidth, wires := rs3_addr },
      ...
      { name := "rs3_phys_out", width := tagWidth, wires := rs3_phys_out },
      { name := "rd_phys_out",  width := tagWidth, wires := rd_phys_out },
      { name := "old_rd_phys_out", width := tagWidth, wires := old_rd_phys }
    ]
  }
