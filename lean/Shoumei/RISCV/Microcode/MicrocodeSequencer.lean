...
/-- Build the microcode sequencer circuit.
    ...
    - is_interrupt_in: interrupt flag (latched on start for mcause override)
    ...
    - mstatus_mret_active: asserted during MSTATUS_MRET µop (CPU handles CSR transform)
...
def mkMicrocodeSequencer : Circuit :=
  ...
  -- Interrupt flag: latched on start, cleared on reset
  let isint_q := Wire.mk "isint_q"
  let isint_d := Wire.mk "isint_d"
  let isintDFF := Gate.mkDFF isint_d clock reset isint_q

  -- Drain done flag: set when DRAIN completes with rob_empty, used to gate speculative flush
  let drain_done_q := Wire.mk "drain_done_q"
  let drain_done_d := Wire.mk "drain_done_d"
  let drain_doneDFF := Gate.mkDFF drain_done_d clock reset drain_done_q
  ...
  let outputGates :=
    [-- fence_i_suppress = active_q (stall fetch while sequencer runs)
     ...
     Gate.mkOR is_read_csr any_mstatus_op read_or_mstatus,
     Gate.mkAND active_q read_or_mstatus active_is_drain,  -- reuse wire name
     Gate.mkAND active_is_drain not_stalling csr_read_en,
     -- csr_write_en = active AND (is_write_csr AND NOT(skip) OR any_mstatus_op) AND not_stalling
