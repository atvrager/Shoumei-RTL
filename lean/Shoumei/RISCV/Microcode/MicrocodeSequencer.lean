...
def mkMicrocodeSequencer : Circuit :=
  ...
  let csraddr_mux := (List.range 12).map (fun i =>
    Gate.mkMUX csraddr_q[i]! csr_addr_in[i]! start csraddr_d[i]!)
  ...
  let temp0Gates :=
    ...
    (List.range 32).map (fun i =>
      Gate.mkMUX t0_read_mux[i]! (Wire.mk "tied_low") start temp0_d[i]!)

  -- temp1_d: on ALU_MOV(dst=1) -> rs1cap, on ALU_OR(dst=1) -> or_result, on ALU_ANDN(dst=1) -> andn_result
  ...
  let temp1Gates :=
    ...
    (List.range 32).map (fun i =>
      Gate.mkMUX temp1_q[i]! rs1cap_q[i]! t1_sel_mov t1_after_mov[i]!) ++
    (List.range 32).map (fun i =>
  ...
  let outputGates :=
    [-- fence_i_suppress = active_q (stall fetch while sequencer runs)
     ...
     -- csr_read_en = active AND is_read_csr AND not_stalling
     Gate.mkAND active_q is_read_csr active_is_drain,  -- reuse wire name
     Gate.mkAND active_is_drain not_stalling csr_read_en,
     -- csr_write_en = active AND is_write_csr AND NOT(skip_write) AND not_stalling
     Gate.mkNOT skipwr_q n_skipwr,
     Gate.mkAND is_write_csr n_skipwr write_en_pre,
     Gate.mkAND active_q write_en_pre active_is_drain_sb,
    ...
    -- csr_write_data = temp1_q (new CSR value)
    (List.range 32).map (fun i => Gate.mkBUF temp1_q[i]! csr_write_data[i]!) ++
    -- csr_addr_out = csraddr_q
    (List.range 12).map (fun i => Gate.mkBUF csraddr_q[i]! csr_addr_out[i]!) ++
    -- fence_i_redir_next: for FLUSH_FETCH use PC+4 (provided externally), for SET_PC use temp
    -- For now, pass through as external input (CPU provides PC+4 on these wires)
    (List.range 32).map (fun i =>
      Gate.mkMUX (Wire.mk s!"redir_pc4_{i}") temp0_q[i]! is_set_pc fence_i_redir_next[i]!)
  ...
  let allGates :=
    ...
    activeGates ++
    rs1cap_mux ++ csraddr_mux ++ rdtag_mux ++ captureBitGates ++
    alu_or_gates ++ alu_andn_gates ++
    temp0Gates ++ temp1Gates ++
    outputGates

  let allInstances := [decoderInst, upcReg, temp0Reg, temp1Reg, rs1capReg, csraddrReg, rdtagReg]
  ...
  { name := "MicrocodeSequencer"
    inputs := [clock, reset, start, Wire.mk "vdd_tie"] ++
              ...
              csr_read_data ++ rom_data ++ redir_pc4 ++ [pipeline_flush]
