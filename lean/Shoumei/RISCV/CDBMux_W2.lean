...
/--
Dual-Issue CDB priority drain mux.
Splits sources across two CDBs so that 2 instructions can complete per cycle.
CDB 0 priority: dmem > lsu > ib_0
CDB 1 priority: fp > muldiv > ib_1
-/
def mkCDBMux_W2 (enableF : Bool) : Circuit :=
  let mk := Wire.mk
  let mkIdx (pfx : String) (n : Nat) := (List.range n).map (fun i => mk s!"{pfx}_{i}")

  let ib_valid_0 := mk "ib_valid_0"
  let ib_deq_0 := mkIdx "ib_deq_0" 72
  let ib_valid_1 := mk "ib_valid_1"
  let ib_deq_1 := mkIdx "ib_deq_1" 72
  ...
  let pre_valid_0 := mk "pre_valid_0"
  let pre_tag_0 := mkIdx "pre_tag_0" 6
  let pre_data_0 := mkIdx "pre_data_0" 32
  let pre_is_fp_0 := mk "pre_is_fp_0"
  let redirect_0 := mkIdx "redirect_0" 32
  let pre_mispred_0 := mk "pre_mispredicted_0"

  let pre_valid_1 := mk "pre_valid_1"
  let pre_tag_1 := mkIdx "pre_tag_1" 6
  let pre_data_1 := mkIdx "pre_data_1" 32
  let pre_is_fp_1 := mk "pre_is_fp_1"
  let redirect_1 := mkIdx "redirect_1" 32
  let pre_mispred_1 := mk "pre_mispredicted_1"

  let drain_lsu := mk "drain_lsu"
  let drain_muldiv := mk "drain_muldiv"
  let drain_fp := mk "drain_fp"
  let drain_ib_0 := mk "drain_ib_0"
  ...
  let ib0_wins_tmp := mk "ib0_wins_tmp"
  let ib0_wins := mk "ib0_wins"
  ...
  let arb0_gates := [
    ...
    Gate.mkBUF ib0_wins drain_ib_0
  ]
  ...
  let is_fp0_gates :=
    if enableF then
      ...
      [Gate.mkBUF zero pre_is_fp_0]

  let redir0_gates := (List.range 32).map (fun i => Gate.mkAND ib_deq_0[39+i]! ib0_wins redirect_0[i]!) ++
  ...
  let ib1_wins_tmp := mk "ib1_wins_tmp"
  let ib1_wins := mk "ib1_wins"
  
  let arb1_gates := [
    Gate.mkBUF fp_valid fp_wins,
    Gate.mkNOT fp_valid not_fp,
    Gate.mkAND muldiv_valid not_fp muldiv_wins,
  ...
  let is_fp1_gates :=
    if enableF then
      ...
      [Gate.mkBUF zero pre_is_fp_1]

  let redir1_gates := (List.range 32).map (fun i => Gate.mkAND ib_deq_1[39+i]! ib1_wins redirect_1[i]!) ++
  ...
  { name := if enableF then "CDBMux_F_W2" else "CDBMux_W2"
    ...
    gates := arb0_gates ++ valid0_gates ++ tag0_gates ++ data0_gates ++ is_fp0_gates ++ redir0_gates ++
             arb1_gates ++ valid1_gates ++ tag1_gates ++ data1_gates ++ is_fp1_gates ++ redir1_gates
    instances := [] }

def cdbMuxW2 : Circuit := mkCDBMux_W2 false
def cdbMuxFW2 : Circuit := mkCDBMux_W2 true
