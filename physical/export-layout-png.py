...
# Colors generated by evenly spacing hue on HSV wheel (s=0.85, v=0.95)
# to guarantee maximum perceptual distance between categories.
# 10 categories at 36° intervals = no two colors within 36° of each other.
MODULE_CATEGORIES = [
    ('Fetch',      (0.14, 0.29, 0.95),  ['fetch', 'auipc_adder', 'jalr_target',    # h=0.64 blue
                                           'br_pc_plus', 'ser_pc_plus',
                                           'insn_queue', 'pc_queue',
                                           'auipc_result']),
    ('Rename',     (0.14, 0.95, 0.29),  ['rename', 'fp_rename', 'busy_bit', 'fp_busy_bit',  # h=0.36 green
                                           'flush_dff_busy', 'busy_reset',
                                           'fp_commit_buf', 'fp_drain_mode']),
    ('Issue',      (0.14, 0.95, 0.73),  ['rs_int', 'rs_memory', 'rs_branch', 'rs_muldiv', 'rs_fp']),  # h=0.45 teal
    ('Exec MulDiv',(0.95, 0.58, 0.14),  ['exec_muldiv']),                           # h=0.09 orange
    ('Exec FP',    (0.44, 0.14, 0.95),  ['exec_fp', 'fflags_dff', 'frm_dff']),      # h=0.73 purple
    ('Memory',     (0.88, 0.95, 0.14),  ['exec_memory', 'lsu', 'dmem_', 'mem_addr_r', 'mem_size_r',  # h=0.18 yellow
                                           ...
                                           'dmem_req_addr', 'dmem_req_size']),
    ('Exec Int',   (0.95, 0.14, 0.14),  ['exec', 'br_cmp', 'br_target',             # h=0.00 red
                                           'br_eq', 'br_gt', 'br_gtu', 'br_lt', 'br_ltu',
                                           'br_captured', 'br_final_target', 'br_imm_rf',
                                           'br_pc_rf', 'br_pred', 'exec_result']),
    ('ROB',        (0.95, 0.14, 0.58),  ['rob', 'redir_tgt_dff', 'redirect_target_dff',  # h=0.91 pink
                                           'redirect_valid_dff', 'rob_isStore_dff']),
    ('CDB',        (0.14, 0.73, 0.95),  ['cdb_']),                                  # h=0.55 cyan
    ('CSR/Trap',   (0.88, 0.14, 0.95),  ['csr_', 'mcycle', 'minstret', 'mscratch', 'mcause',  # h=0.82 magenta
...
def classify_instance(name: str) -> str | None:
    ...
    DEF naming conventions (after Yosys flatten):
      u_cpu/u_cpu.u_<submodule>.<detail>$_DFF_...  — named submodule FFs
      u_cpu/u_cpu.<signal>...                       — top-level named signals/FFs
      u_cpu/_NNNNN_                                 — anonymous combo gates

    Returns category name or None if not a u_cpu component.
    """
    # Handle both / and . hierarchy separators
    if name.startswith('u_cpu/'):
        rest = name[6:]  # strip "u_cpu/"
    elif name.startswith('u_cpu.'):
        rest = name[6:]  # strip "u_cpu."
    else:
        return None

    # Anonymous combo gates: u_cpu/_NNNNN_
    if rest.startswith('_') and not rest.startswith('_cpu'):
        return 'Dispatch'

    # Strip "u_cpu." prefix if present (from u_cpu/u_cpu.u_foo format)
    if rest.startswith('u_cpu.'):
        rest = rest[6:]

    # Named submodule: u_<submodule>...
    if rest.startswith('u_'):
        sub = rest[2:]  # strip "u_"
        # Trim everything after first . or [ or $ (instance detail)
        for sep in '.[$\\':
            idx = sub.find(sep)
            if idx >= 0:
                sub = sub[:idx]
    ...
    # Top-level signal (no u_ prefix) — match by signal name
    for cat_name, _, prefixes in MODULE_CATEGORIES:
        for prefix in prefixes:
...
if density_maps:
    ...
    peak_idx = np.unravel_index(np.argmax(total_density), total_density.shape)
    peak_y, peak_x = int(peak_idx[0]), int(peak_idx[1])
else:
    peak_y, peak_x = GRID_N // 2, GRID_N // 2
...
# Shorten design name for stats panel
stats_design = design_name.replace('_synth', '')
stats_text = [
    ('DESIGN',      stats_design),
    ('ISA',         f'{isa} W=2 OoO'),
    ('PROCESS',     pdk_display),
    ...
    ('CELLS',       f'{stdcells:,} ({seq_cells:,} seq)'),
    ('UTIL',        f'{util*100:.0f}%'),
    ('POWER',       f'{power:.1f} mW'),
    ('DIE',         f'{chip_w:.0f} \u00d7 {chip_h:.0f} \u00b5m\u00b2'),
    ('PROOF',       'Lean 4 verified'),
]

y = 0.95
for label, value in stats_text:
    ax_stats.text(0.03, y, label, color='#6677aa', fontsize=7.5,
                  fontfamily='monospace', fontweight='bold', va='top')
    ax_stats.text(0.38, y, value, color='white', fontsize=7.5,
...
# Shorten design name for title (keep full name in stats panel)
short_name = design_name.replace('_synth', '').replace('Cached', '').replace('_', ' ')
title_name = f'Shoumei RTL \u2014 {short_name}'
subtitle_parts = [
    mode_label,
    'W=2 superscalar',
    'Lean 4 verified',
...
if cjk_font:
    fig.text(0.5, 0.97, f'\u8a3c\u660e  {title_name}', color='white', fontsize=22,
             fontweight='bold', ha='center', va='top', fontproperties=cjk_font)
else:
    fig.text(0.5, 0.97, title_name, color='white', fontsize=22,
             fontweight='bold', ha='center', va='top', fontfamily='sans-serif')
fig.text(0.5, 0.945, subtitle,
         color='#8899bb', fontsize=11, ha='center', va='top')
...
if args.mode == 'modules':
    ...
    for cat, pts in cats_sorted:
        ...
        hex_c = '#{:02x}{:02x}{:02x}'.format(int(c[0]*255), int(c[1]*255), int(c[2]*255))
        n = len(pts)
        count_str = f'{n/1000:.1f}k' if n >= 1000 else str(n)
        legend_items.append(Patch(facecolor=hex_c, alpha=0.8, label=f'{cat} ({count_str})'))
else:
    layers_by_size = sorted(sorted_layers, key=lambda l: len(by_layer[l]), reverse=True)
...
ncols = 3 if (args.mode == 'modules' and len(legend_items) > 6) else (2 if args.mode == 'modules' else 1)
ax_main.legend(handles=legend_items, loc='upper right', fontsize=7,
               facecolor='#0c0c18ee', edgecolor='#334466', labelcolor='white',
               ncol=ncols, columnspacing=1.0, handletextpad=0.5)
